...
export default async function ExamenPage() {
  ...
  return (
    <main className="flex-1 px-6 py-10 max-w-3xl mx-auto w-full space-y-8">
      ...
      {esAdmin ? (
        <>
          <div className="rounded-2xl bg-cafe/10 border border-border px-5 py-3 text-xs text-cafe/70">
            <span className="uppercase tracking-widest text-terracota font-medium">
              Vista admin
            </span>{" "}
            — puedes tomar el examen sin límites. Estado real del vendedor
            promedio con tu perfil:{" "}
            {certificado
              ? "certificado"
              : proximoIntento
