...
function LoginInner() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      try {
        if (localStorage.getItem(VISITED_KEY) === "1") setReturning(true);
      } catch {}
  ...
  if (checkingSession) {
    return (
      <main className="flex-1 flex items-center justify-center">
        <p className="text-sm text-cafe/60">Cargando…</p>
      </main>
    );
  }

  const primaryLabel = returning ? "Entrar" : "Continuar con Google";
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center px-4 py-8">
      <div className="relative w-full max-w-[640px] aspect-[16/10] rounded-[32px] overflow-hidden bg-crema shadow-2xl border border-border select-none">
        ...
        {/* RIGHT — button revealed underneath */}
        <div className="absolute inset-y-0 right-0 w-1/2 flex flex-col items-center justify-center gap-4 px-4 sm:px-6 text-center">
          ...
          <button
            onClick={signIn}
            disabled={loading}
            className="inline-flex items-center justify-center gap-2 rounded-full bg-cafe px-6 py-3 text-crema text-sm font-medium hover:bg-cafe/90 disabled:opacity-60 transition"
          >
            {!returning && <GoogleIcon />}
            {loading ? "Redirigiendo…" : primaryLabel}
          </button>
          {(error || urlError) && (
        ...
        {/* FLAP — covers the right half, tilts to reveal */}
        <button
          type="button"
          onClick={() => setRevealed(true)}
          disabled={revealed}
          aria-label={returning ? "Revelar botón para entrar" : "Revelar botón de sign in"}
