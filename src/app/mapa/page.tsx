...
export default async function MapaPage() {
  ...
  return (
    <main className="flex-1 px-6 py-10 max-w-5xl mx-auto w-full space-y-6">
      ...
      {mapaError ? (
        <div className="rounded-2xl border border-border bg-white/60 p-6 space-y-2 text-sm">
          <p className="font-medium text-terracota-oscuro">
            No se pudo conectar al Mapa.
          </p>
          <p className="text-cafe/70">{mapaError}</p>
          <p className="text-xs text-cafe/60">
            Verifica que las env vars <code className="bg-cafe/10 px-1 rounded">MAPA_PROSPECTOS_API_URL</code>{" "}
            y <code className="bg-cafe/10 px-1 rounded">MAPA_PROSPECTOS_API_KEY</code>{" "}
